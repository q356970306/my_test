 ...
 */
function login(){
    var url='/my-test/zhixing/create',
        data={
...
function getImgCode() {
    var url='/my-test/zhixing/status',
        data={
            tradeNo:$("#tradeNo").val(),
            testType:$("#project_select").val(),
        };
...
function notify(){
    window.open("notify.html?taskId="+$("#taskId").val() +"&project="+$("#project_select").val()+"&taskType=ZHIXINGCOURT" +
                "&msg=" + $("#msg").val());
}

function query(){
    window.open("../../work/zhixing.html?tradeNo="+$("#tradeNo").val()+"&testType="+$("#project_select").val());
}